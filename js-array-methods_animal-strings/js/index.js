...
const catStartingFromIndexFiveExists =
  animalStrings.slice(5).findIndex((animal) => animal === "cat") !== -1;
...
//get the length of each animal name
const allAnimalCharacters = animalStrings.map((animal) => animal.length)
console.log(allAnimalCharacters);
//sum up those lengths into a single total
const sumOfAllAnimalCharacters = allAnimalCharacters.reduce((sum, length) => sum + length, 0);
